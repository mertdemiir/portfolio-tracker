...
export function WelcomeScreen({ onSaveKey, onSkip }: WelcomeScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-4">
      <div className="bg-surface-card rounded-2xl shadow-lg w-full max-w-md p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 mb-4">
          <input
            ...
            className="w-full px-4 py-3 border border-b-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          />
          <button
            type="submit"
            disabled={!keyInput.trim()}
            className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-40 transition-colors"
          >
            Save & continue
