 ...
 *
 *   buy / sell         — classic position movement. Updates shares + cost basis.
 *   deposit            — cash in (payroll, transfer, dividend reinvest-as-cash).
 *                        Ticker is the cash holding's ticker; shares is the
 *                        cash amount; pricePerShare is 1 (in holding currency).
 *   withdrawal         — cash out (spent, transferred out).
 *   interest           — interest/yield credit on a cash or bond-like holding.
 *   correction         — manual adjustment to reconcile the ledger with reality
 *                        (e.g. stock split not yet auto-handled, reconciliation).
 *
 * Phase 3.1 added the cash/ledger types alongside the existing buy/sell set.
 * Holdings remain authoritative for now — the deriveHolding switch is gated
 * behind a future phase. See src/data/migrations.ts (migration 3) for the
 * synthetic-buy backfill that guarantees every Holding has at least one
 * matching buy Transaction.
 */
export type TransactionType = 'buy' | 'sell' | 'deposit' | 'withdrawal' | 'interest' | 'correction';
...
export interface Transaction {
  ...
  portfolioId: string;
  /**
   * Explicit link back to the Holding this transaction belongs to.
   * Backfilled in schema v3 migration. Prefer this over (ticker, portfolioId)
   * matching when present — two holdings in the same bucket with the same
   * ticker (rare but possible after manual import) would otherwise collide.
   */
  holdingId?: string;
  /**
   * FX rate at transaction time: 1 unit of `currency` = buyFxRate units of
   * base currency. Used to keep cost basis stable when base currency changes.
   * Optional for backward compatibility — legacy transactions may not have it.
   */
  buyFxRate?: number;
  /**
   * True if this transaction was generated by the v3 migration to provide
   * 1:1 parity between Holdings and buy Transactions. Synthetic buys carry
   * the holding's current buyPrice/shares/buyDate snapshot and should be
   * treated as representative, not historically accurate.
   */
  synthetic?: boolean;
}
